...
const FinancialView: React.FC = () => {
  ...
  const filteredRecords = useMemo(() => {
    return MOCK_FINANCIALS.filter(record => {
  ...
  const getPatientName = (patientId: string) => {
    ...
    return MOCK_PATIENTS.find(p => p.id === patientId)?.name || 'Desconhecido';
  };
            ...
            <form>
                ...
                <div className="mb-4">
                 ...
                 <div className="mb-4">
                    ...
                    <input type="number" step="0.01" required placeholder="Use negativo para despesas (ex: -50.00)" className="w-full p-2 bg-white border border-gray-300 rounded"/>
                </div>
                 <div className="mb-4">
                    <label className="block text-gray-700">Data</label>
                    <input type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-2 bg-white border border-gray-300 rounded"/>
                </div>
                 <div className="mb-4">
                ...
                <div className="flex justify-end space-x-4">
                    ...
                    <button type="submit" onClick={(e) => { e.preventDefault(); alert('Lançamento salvo!'); setIsModalOpen(false); }} className="px-4 py-2 text-white bg-green-500 rounded-md">Salvar</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div className="p-4 bg-white rounded-lg shadow-md printable-area">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white">
                 ...
                 {filteredRecords.length === 0 && <p className="mt-4 text-center text-gray-500">Nenhuma transação encontrada para o período selecionado.</p>}
            </div>
        </div>
    </div>
  );
