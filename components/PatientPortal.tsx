...
import { MOCK_EVOLUTIONS, MOCK_USERS, MOCK_CHAT_MESSAGES } from '../constants';
...
const PatientPortal: React.FC<PatientPortalProps> = ({ patient, onLogout }) => {
    const externalEvolutions = MOCK_EVOLUTIONS.filter(e => e.patientId === patient.id && !e.isInternal).sort((a,b) => b.date.getTime() - a.date.getTime());
    
    const assignedTherapist = MOCK_USERS.find(u => u.role === 'THERAPIST');
    ...
    const getTherapistName = (therapistId: string) => MOCK_USERS.find(u => u.id === therapistId)?.name || 'Terapeuta';
                ...
                <main className="grid grid-cols-1 gap-8 mt-8 lg:grid-cols-2">
                    ...
                    <div className="flex flex-col h-[70vh] bg-white rounded-lg shadow-md">
                        <div className="p-4 border-b">
                            <h2 className="text-xl font-bold text-gray-800">Chat com {assignedTherapist?.name}</h2>
                        </div>
