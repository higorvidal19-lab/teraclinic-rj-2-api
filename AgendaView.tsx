...
import { UserRole, AppointmentRecurrence, BillingType } from '../types';
...
const AgendaView: React.FC<AgendaViewProps> = ({ user }) => {
  const { appointments, setAppointments, patients, users } = useClinic();
  ...
  }, [selectedTherapist, user, appointments]);

  const getPatientName = (patientId: string) => patients.find(p => p.id === patientId)?.name || 'Desconhecido';
  ...
  const AppointmentModal: React.FC<{ appointment?: Appointment | null, onClose: () => void }> = ({ appointment, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Correctly create date in local timezone to avoid timezone shift issues
        const startDateTime = new Date(year, month - 1, day, hours, minutes, 0, 0);

        const endDateTime = new Date(startDateTime.getTime() + 50 * 60000); // 50 minutes duration
        ...
        if (isEditing) {
            ...
            const newAppointment: Appointment = {
                ...
                billing: { type: BillingType.PER_SESSION, amount: 0, paid: false } // Default billing
            };
            ...
            <div className="w-full max-w-lg p-6 bg-white rounded-lg shadow-xl">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        ...
                        <select name="patientId" defaultValue={appointment?.patientId} className="w-full p-2 bg-white border border-gray-300 rounded" required>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input name="date" type="date" className="w-full p-2 bg-white border border-gray-300 rounded" defaultValue={appointment?.start.toISOString().split('T')[0] || selectedDay.toISOString().split('T')[0]} required />
                        </div>
                        <div>
                            <label className="block text-gray-700">Horário</label>
                            <input name="time" type="time" step="300" className="w-full p-2 bg-white border border-gray-300 rounded" defaultValue={appointment ? formatTimeForInput(appointment.start) : ''} required />
                        </div>
                    </div>
                     <div className="mb-4">
                        <label className="block text-gray-700">Terapeuta</label>
                        <select name="therapistId" defaultValue={appointment?.therapistId} className="w-full p-2 bg-white border border-gray-300 rounded" required>
                            {therapists.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>
                    </div>
                    <div className="mb-4">
                        <label className="block text-gray-700">Tipo de Terapia</label>
                         <input name="title" type="text" className="w-full p-2 bg-white border border-gray-300 rounded" defaultValue={appointment?.title} placeholder="Ex: Sessão T.O." required/>
                    </div>
