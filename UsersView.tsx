...
const UsersView: React.FC<UsersViewProps> = ({ currentUser }) => {
    const { users, setUsers, settings } = useClinic();
    ...
    const UserModal: React.FC<{ user: User | null, onClose: () => void }> = ({ user, onClose }) => {
        ...
        const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
            ...
            const updatedUser = {
                id: isEditing ? user.id : `user-${Date.now()}`,
                name: formData.get('name') as string,
                email: formData.get('email') as string,
                role: role,
                councilType: formData.get('councilType') as string,
                councilNumber: formData.get('councilNumber') as string,
            ...
            alert(`Usuário ${isEditing ? 'atualizado' : 'salvo'} com sucesso!`);
            onClose();
        };
                ...
                <div className="w-full max-w-2xl p-6 bg-white rounded-lg shadow-xl max-h-[90vh] overflow-y-auto">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="flex items-center space-x-4">
                            <Avatar name={user?.name || 'Novo'} avatarUrl={avatarPreview} size="lg"/>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Foto de Perfil</label>
                                <input type="file" ref={avatarFileRef} onChange={handleAvatarChange} accept="image/*" className="hidden"/>
                                <button type="button" onClick={() => avatarFileRef.current?.click()} className="mt-1 text-sm text-green-600 hover:underline">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="name" type="text" required={!isEditing} defaultValue={user?.name} className="w-full p-2 bg-white border border-gray-300 rounded"/>
                            </div>
                            <div>
                                <label className="block text-gray-700">Email</label>
                                <input name="email" type="email" required={!isEditing} defaultValue={user?.email} className="w-full p-2 bg-white border border-gray-300 rounded"/>
                            </div>
                             {!isEditing && (
                                <>
                                    <div>
                                        <label className="block text-gray-700">CPF</label>
                                        <input name="cpf" type="text" required className="w-full p-2 bg-white border border-gray-300 rounded"/>
                                    </div>
                                    <div>
                                        <label className="block text-gray-700">Endereço</label>
                                        <input name="address" type="text" className="w-full p-2 bg-white border border-gray-300 rounded"/>
                                    </div>
                                    <div>
                                        <label className="block text-gray-700">Senha</label>
                                        <input name="password" type="password" required className="w-full p-2 bg-white border border-gray-300 rounded"/>
                                    </div>
                                </>
                            )}
                             <div>
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                     <div>
                                        ...
                                        <select name="councilType" defaultValue={user?.councilType} className="w-full p-2 bg-white border border-gray-300 rounded">
                                            {therapistCouncils.map(c => <option key={c} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-gray-700">Número do Conselho</label>
                                        <input name="councilNumber" type="text" defaultValue={user?.councilNumber} className="w-full p-2 bg-white border border-gray-300 rounded"/>
                                    </div>
                                </div>
                            </>
                        )}
                            ...
                            {displayUsers.map(user => (
                                <tr key={user.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="py-4 px-6 whitespace-nowrap text-sm font-medium">
                                        ...
                                        <button onClick={() => handleRemove(user.id)} className="ml-4 text-red-600 hover:text-red-900">Remover</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    {displayUsers.length === 0 && <p className="text-center py-4 text-gray-500">Nenhum outro usuário cadastrado.</p>}
                </div>
            </div>
        </div>
    );
